...
TEST(t_start_end)
{
	...
	t_set_data(tmp);
	t_start();
	res = t_end(1, "filde1", "func", 0);
	t_set_data(data);
	EXPECT_EQ(res, 0);
	EXPECT_STR(buf, "├─" CG "PASS filde1" CW "\n");

	t_set_data(tmp);
	mem_stats_alloc(1);
	t_start();
	res = t_end(1, "filde2", "func", 0);
	mem_stats_free(1);
	t_set_data(data);
	EXPECT_EQ(res, 1);
	EXPECT_STR(buf,
...
TEST(t_expect_g)
{
	...
	t_set_data(tmp);
	t_expect_g(0, NULL, NULL, 0, NULL, 1, NULL, 1, "==", UINT8_MIN, UINT8_MAX);
	t_set_data(data);
	EXPECT_STR(buf, "│ " CR "(null) == (null) (00000000 == 11111111)" CW "\n");

	t_set_data(tmp);
	t_expect_g(0, NULL, NULL, 0, NULL, 2, NULL, 2, "==", UINT16_MIN, UINT16_MAX);
	t_set_data(data);
	...
	t_set_data(tmp);
	t_expect_g(0, NULL, NULL, 0, NULL, 4, NULL, 4, "==", UINT32_MIN, UINT32_MAX);
	t_set_data(data);
	EXPECT_STR(buf, "│ " CR "(null) == (null) (00000000 == FFFFFFFF)" CW "\n");

	t_set_data(tmp);
	t_expect_g(0, NULL, NULL, 0, NULL, 8, NULL, 8, "==", UINT64_MIN, UINT64_MAX);
	t_set_data(data);
...
TEST(t_expect_str_passed)
{
	...
	t_set_data(tmp);
	t_expect_str(1, "file", "func", 0, NULL, NULL);
	t_set_data(data);
	EXPECT_STR(buf,
		   "│ ├─" CR "FAIL file" CW "\n"
		   "│ │ " CR "file:0: " CW "\n"
...
TEST(t_expect_wstr_passed)
{
	...
	t_set_data(tmp);
	t_expect_wstr(1, "file", "func", 0, NULL, NULL);
	t_set_data(data);
	EXPECT_STR(buf,
		   "├─" CR "FAIL file" CW "\n"
		   "│ " CR "file:0: " CW "\n"
